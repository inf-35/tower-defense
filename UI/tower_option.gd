...
func display_tower_type(tower_type : Towers.Type) -> void:
...
func _refresh_state() -> void:
	...
	var gold_text: String = "{GOLD|icon_size=40%s} %s" % ["|color=red" if not has_gold else "", str(cost)]
	if not has_gold:
		gold_text = "[color=red]%s[/color]" % gold_text

	var capacity_text: String = "{POPULATION|icon_size=40%s} %s" % ["|color=red" if not has_capacity else "", str(capacity_cost)]
	if not has_capacity:
		capacity_text = "[color=red]%s[/color]" % capacity_text
