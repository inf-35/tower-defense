...
func _on_mouse_entered() -> void:
	...
	if _entry.is_combat:
		...
		var variant_name: String = Run.phases.CombatVariant.keys()[_entry.subtype].capitalize()
		...
		#variant name
		var event_name: String = Run.phases.DayEvent.keys()[_entry.subtype].capitalize()
		desc_text = "[color=#55cc55]%s[/color]" % event_name

		#flavour text
		if _entry.subtype == Run.phases.DayEvent.EXPANSION:
			desc_text += "\nExpand your territory."
		elif _entry.subtype == Run.phases.DayEvent.REWARD_TOWER:
