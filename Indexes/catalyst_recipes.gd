extends Node #catalystrecipes is technically a singleton, but it behaves more like a shared data table
...
class CatalystRecipe:
	...
	#this stays keyed by element so same-element neighbors always resolve to the same catalyst outcome

	func _init(_requisites: Dictionary[Towers.Element, int], _effects: Dictionary[Towers.Element, EffectPrototype]) -> void:
...
func get_most_relevant_recipe(element_list: Dictionary[Towers.Element, int]) -> CatalystRecipe:
