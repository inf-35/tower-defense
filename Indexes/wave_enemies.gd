...
static func get_enemies_for_wave(wave: int) -> Array[Array]:
	if SCRIPTED_WAVES.has(wave):
		...
		for i in output.size():
			output[i][1] = round(float(output[i][1]) * Run.current_game_scaling)

		return output

	#1. get the wave modifier from the central game director
	var wave_type: Phases.CombatVariant = Run.phases.get_combat_variant(wave)
	var modifier: WaveModifier

	match wave_type:
		Run.phases.CombatVariant.NORMAL:
			modifier = WaveModifier.MIXED_ARMS
		Run.phases.CombatVariant.BOSS:
			modifier = WaveModifier.ELITE
		Run.phases.CombatVariant.SURGE:
