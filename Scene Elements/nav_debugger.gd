...
func _ready() -> void:
	...
	if is_instance_valid(Run.references.island):
		Run.references.island.navigation_grid_updated.connect(queue_redraw)
...
func _draw_grid_state() -> void:
	...
	for cell: Vector2i in Navigation.grid:
		...
		var rect: Rect2 = Rect2(pos - Vector2(cell_size, cell_size)/2.0, Vector2(cell_size, cell_size))
...
func _draw_simulation_overlay() -> void:
	var cell_size = Island.CELL_SIZE
	var half: Vector2 = Vector2(cell_size, cell_size) / 2.0
