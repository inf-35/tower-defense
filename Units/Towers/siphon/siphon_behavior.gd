...
func start() -> void:
	...
	Player.on_event.connect(_on_global_event)
...
func _subscribe() -> void:
	if _subscription_id != -1 or not is_instance_valid(References.island) or not is_instance_valid(References.island.topology_service):
		return
	_subscription_id = References.island.topology_service.subscribe(self, unit as Tower, _make_query(), _on_topology_updated, true)
