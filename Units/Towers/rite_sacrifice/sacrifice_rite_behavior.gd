...
func _on_target_hit(event: GameEvent, victim: Tower) -> void:
	...
	var hit: HitData = event.data as HitData
...
func _exit_tree() -> void:
	...
	if _subscription_id != -1 and is_instance_valid(Run.references.island) and is_instance_valid(Run.references.island.topology_service):
