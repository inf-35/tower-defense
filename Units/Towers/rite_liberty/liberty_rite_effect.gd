...
func _handle_detach(instance: EffectInstance) -> void:
	var state: LibertyState = instance.state as LibertyState
...
func _update_bonus(instance: EffectInstance) -> void:
	...
	for cell: Vector2i in adjacent_cells:
		if not Run.references.island.terrain_base_grid.has(cell):
			continue

		if Run.references.island.get_tower_on_tile(cell) == null:
			empty_count += 1
