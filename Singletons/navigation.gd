...
func _find_path_internal(start: Vector2i, goal: Vector2i, ignore_walls: bool, extra_blockers: Array[Vector2i] = []) -> PathData:
	var local_flow_field: Dictionary = {}
	var open_set := PriorityQueue.new([ [_heuristic(goal, goal), goal] ])
	var g_score: Dictionary = { goal: 0.0 }
	local_flow_field[goal] = Vector2i.ZERO
	...
	while curr != goal:
		if not local_flow_field.has(curr): break
		var d = local_flow_field[curr]
		curr += d
		path.append(curr)
...
func get_hypothetical_path(start: Vector2i, goal: Vector2i, blocker_cells: Array[Vector2i], ignore_walls: bool = false) -> PathData:
	# Reuse the internal logic but pass the blockers
