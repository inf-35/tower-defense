...
class TerrainGenRule extends Resource:
	...
	func _init(_type: Terrain.Base = Terrain.Base.EARTH, _prob: float = 0.0, _c_min: int = 1, _c_max: int = 1) -> void:
		terrain_type = _type
		probability = _prob
...
func start_expansion_phase() -> void: #entry point called by phases
	generate_and_present_choices(Run.references.island, 22, 4)
...
func select_expansion(island: Island, choice_id: int) -> void:
	...
	if expansions_this_phase > 1:
		Run.current_game_scaling += 0.4

	generate_and_present_choices(Run.references.island, 12, 4)
	if expansions_this_phase >= minimum_expansions:
...
func _on_choice_hovered(choice_id: int) -> void:
	...
	Run.references.island.set_highlighted_choice(choice_id)
...
func _on_choice_unhovered(_choice_id: int) -> void:
	if _current_state != State.CHOOSING:
		return
	_hovered_choice_id = -1
	Run.references.island.set_highlighted_choice(-1)
...
func _on_expansion_option_clicked(choice_id: int) -> void:
	...
	Run.references.island.set_highlighted_choice(choice_id)
	_trigger_camera_focus_on_choice(Run.references.island, choice_id)
...
func _on_expansion_confirmed(_choice_id: int) -> void:
	...
	select_expansion(Run.references.island, _pending_choice_id)

func _on_exit() -> void:
	_clear_expansion_state(Run.references.island)
	expansion_process_complete.emit()
...
func _generate_block(island: Island, block_size: int, params: GenerationParameters) -> Dictionary[Vector2i, Terrain.CellData]:
	Run.references.terrain_generating.emit(params)
...
func _trigger_camera_focus_on_choice(_island: Island, choice_id: int) -> void: ##snaps once to the currently focused expansion option without suppressing manual camera control
	...
	var choice_bounds: Rect2 = Rect2(choice.block_data.keys()[0], Vector2.ONE)
