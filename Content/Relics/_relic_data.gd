...
#used only if target_type is SPECIFIC_ELEMENT
@export var specific_element: Towers.Element
# --- effect data ---
# each relic contains a modifier prototype that defines its stat changes
@export var modifier_prototype: Array[ModifierDataPrototype] ##modifier that is applied to all satisfying towers (see specific_tower_type)
